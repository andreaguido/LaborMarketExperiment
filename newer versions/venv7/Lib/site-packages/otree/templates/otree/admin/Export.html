  ...
  {% else %}
    ...
    <p>
      ...
      <a href="javascript:void(0)" class="download-link"
         id="wide-xlsx"
         data-file-extension="xlsx">Excel</a>
      |
      <a href="javascript:void(0)" class="download-link"
         id="wide-csv"
         data-file-extension="csv">CSV</a>
    </p>
    <p>
...
{% block scripts %}
  <script>
      ...
      function saveReceivedFile(content) {
          var c = content;
          if (c.file_extension === 'xlsx') {
              var data = s2ab(atob(c.data));
          } else {
              var data = c.data;
          }
